...
export default function ActivitiesGridPage() {
  ...
  const filteredResidents = useMemo(() => {
    ...
    return rows
  }, [residents, search, roomFilter, rooms])

  const visibleResidents = filteredResidents
  const visibleResidentIds = useMemo(
    () => new Set(visibleResidents.map((resident) => resident.id)),
    [visibleResidents]
  )
  ...
  }, [visibleResidents, sessions, attendanceMap, duplicatedNames, rooms])
  ...
  async function saveGridChanges(changes: Array<[number, number | string, unknown, unknown]>) {
    ...
    for (const [rowIndex, prop, _oldValue, newValue] of changes) {
      ...
      if (!visibleResidentIds.has(resident.id)) continue
    ...
    if (error) {
      ...
      setMessage(`Išsaugota pakeitimų: ${payload.length}`)
  ...
  async function handleAfterChange(
    changes: Array<[number, number | string, unknown, unknown]> | null,
    source: string
  ) {
          ...
          {sessions.length === 0 ? (
            ...
            <HotTable
              ...
              afterChange={(changes, source) => {
                void handleAfterChange(changes as any, source)
              }}
              cells={(row, col) => {
