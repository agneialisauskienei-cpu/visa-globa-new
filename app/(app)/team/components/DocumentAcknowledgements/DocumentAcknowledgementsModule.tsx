...
function makeMissingDocuments(
  ...
  requiredDocuments: string[] = DEFAULT_REQUIRED_DOCUMENTS,
) {
  const safeEmployees = Array.isArray(employees) ? employees : []
  const safeCredentials = Array.isArray(credentials) ? credentials : []
  const safeRequiredDocuments = Array.isArray(requiredDocuments) && requiredDocuments.length ? requiredDocuments : DEFAULT_REQUIRED_DOCUMENTS
  const rows: MissingDocumentRecord[] = []

  for (const employee of safeEmployees) {
    for (const required of safeRequiredDocuments) {
      const exists = safeCredentials.some(
        (credential) =>
...
export default function DocumentsModule({
  ...
  requiredDocuments: requiredDocumentsInput = DEFAULT_REQUIRED_DOCUMENTS,
  onRefresh,
  ...
  const employees = useMemo(
    () => (Array.isArray(employeesInput) ? employeesInput : []),
    [employeesInput],
  )
  const credentials = useMemo(
    () => (Array.isArray(credentialsInput) ? credentialsInput : []),
    [credentialsInput],
  )
  const requiredDocuments = useMemo(
    () =>
      Array.isArray(requiredDocumentsInput) && requiredDocumentsInput.length
  ...
  return (
    <section className="rounded-lg border border-[#dbe6e0] bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.85fr_1.35fr]">
        <div id="documents-form" className="rounded-xl border border-[#dbe6e0] bg-[#ffffff]/70 p-5">
