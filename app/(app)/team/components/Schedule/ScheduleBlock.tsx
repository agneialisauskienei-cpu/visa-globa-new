...
function emptySettings(): TemplateSettings {
  return { templates: {}, patterns: {}, allowTwentyFourHourDuty: {} };
}
...
export default function ScheduleBlock({
  ...
  const [templateDraft, setTemplateDraft] = useState("08-17 P30");
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setSettings({ ...emptySettings(), ...JSON.parse(stored) });
    } catch {}
  }, []);
  ...
  const filteredEmployees = useMemo(() => {
    ...
    if (filter === "reservations") return employees.filter((employee) => vacationReservations.some((reservation) => reservation.employee_id === employee.user_id));
  ...
  }, [employees, filter, vacationReservations, validationByEmployee]);

  const gridTemplateColumns = useMemo(() => `minmax(180px, 220px) repeat(${scheduleDays.length}, minmax(42px, 1fr)) minmax(92px, 112px)`, [scheduleDays.length]);
  ...
  const quickSet = (value: string) => {
    ...
    commitCell(activeCell.row, activeCell.col, oldValue, value);
  };

  const defaultTemplates = ["08:00-17:00 P60", "08:00-17:00 P30", "08:00-16:00 P30", "07:00-19:00 P30", "19:00-07:00", "07:00-07:00"];
  const employeeTemplates = activeEmployee ? Array.from(new Set([...defaultTemplates, ...(settings.templates[activeEmployee.user_id] || [])])) : defaultTemplates;

  const addTemplate = () => {
    const employee = employees.find((item) => item.user_id === templateEmployeeId) || activeEmployee;
    if (!employee) return;
    const parsed = parseShiftValue(templateDraft);
    if (!(parsed.kind === "work" || parsed.kind === "night")) {
      setMessage("Šablonui įveskite pamainos laiką, pvz. 08-17, 08-17 P30 arba 07:00-07:00.");
      return;
    }
    setSettings((prev) => ({
      ...prev,
      templates: { ...prev.templates, [employee.user_id]: Array.from(new Set([parsed.normalized, ...(prev.templates[employee.user_id] || [])])).slice(0, 12) },
    }));
    setTemplateDraft(parsed.normalized);
    setMessage(`${getName(employee)}: pridėtas šablonas ${parsed.normalized}`);
  };
  ...
  const useTemplate = (template: string) => {
    const parsed = parseShiftValue(template);
    const normalized = parsed.normalized || template;

    if (activePatternDay !== null && selectedPatternEmployee) {
      updatePattern(selectedPatternEmployee.user_id, activePatternDay, normalized);
      setTemplateDraft(normalized);
      setMessage(`${WEEKDAYS.find((item) => item.day === activePatternDay)?.label || "Diena"}: įrašyta ${normalized}`);
      return;
    }
  ...
  const updatePattern = (employeeId: string, day: number, value: string, normalize = false) => {
    ...
    const parsed = parseShiftValue(trimmed);
    const nextValue = !trimmed ? "" : normalize && (parsed.kind === "work" || parsed.kind === "night") ? parsed.normalized : value;
    setSettings((prev) => ({
      ...prev,
      patterns: {
        ...prev.patterns,
  ...
  const applyPattern = (mode: "selected" | "all") => {
    ...
    targetEmployees.forEach((employee) => {
      ...
      scheduleDays.forEach((date, dayIndex) => {
        ...
        const parsedPattern = parseShiftValue(rawPatternValue);
  ...
  const autofillWorkdays = () => {
    ...
    employees.forEach((employee, row) => {
      const training = getTrainingStatus(employee);
      if (training.blocking) return;
      ...
      scheduleDays.forEach((date, dayIndex) => {
        ...
        const fallbackValue = isPreHoliday(date) ? "08:00-16:00 P60" : "08:00-17:00 P60";
        const parsed = parseShiftValue(patternValue || fallbackValue);
    ...
    queueChanges(
      ...
      `Užpildyta darbo dienų: ${changes.length}. Pildoma pagal darbuotojo darbo laiko modelį, o jei jo nėra — 08:00-17:00 P60.`
    );
  };
  ...
  const exportSchedule = () => {
    const rows = [
      ["Darbuotojas", "Pareigybė", ...scheduleDays.map((date) => `${pad2(date.getDate())} ${weekdayLabel(date)}`), "Iš viso", "Statusas"],
      ...employees.map((employee, row) => {
        const validation = validationByEmployee.get(employee.user_id);
        return [getName(employee), getRole(employee), ...scheduleDays.map((_, dayIndex) => parseShiftValue(getValue(row, dayIndex + 1)).normalized), `${formatHours(validation?.plannedHours || 0)} val.`, validation?.statusLabel || "Be neatitikimų"];
      }),
    ];
  ...
  const visibleCountText = `Rodoma: ${filter === "all" ? "visi darbuotojai" : filter === "errors" ? "darbuotojai su pažeidimais" : filter === "warnings" ? "darbuotojai su įspėjimais" : filter === "training" ? "mokymų rizikos" : "neatvykimų rezervacijos"} (${filteredEmployees.length} iš ${employees.length}).`;
    ...
    <section className="schedule-module" onMouseUp={finishDrag}>
      <div className="schedule-toolbar">
        <div>
          ...
          <p className="subtle">Tikrinamas darbo laikas, poilsis, neatvykimai, mokymai, pertraukos ir prieššventinės dienos.</p>
        </div>
      ...
      <div className="panel controls-panel">
        <div className="quick-actions">
          ...
          <button type="button" className="pill" onClick={() => quickSet("08-17 P30")}><Coffee size={13} /> 08-17 P30</button>
          <button type="button" className="pill" onClick={() => quickSet("")}>Išvalyti</button>
        </div>
        <div className="help-text">Kopijavimas: pasirinkite langelį, naudokite <b>Ctrl+C</b> / <b>Ctrl+V</b> arba tempkite užpildytą langelį per kitus langelius.</div>
      </div>

      <div className="panel templates-panel">
        <div>
          <h3>Pamainų šablonai</h3>
          <p className="subtle">Pasirinkus grafiko langelį šablonas įrašomas į grafiką. Pasirinkus darbo laiko modelio lauką — į tą savaitės dieną. Pertrauka: <b>P30</b>, pvz. 08-17 P30.</p>
        </div>
        <div className="template-form">
          ...
          <input value={templateDraft} onChange={(event) => setTemplateDraft(event.target.value)} onBlur={(event) => { const parsed = parseShiftValue(event.target.value); if (parsed.kind === "work" || parsed.kind === "night") setTemplateDraft(parsed.normalized); }} placeholder="08-17 P30" />
        ...
        <div className="pattern-grid">
          {WEEKDAYS.map((day) => (
            <label key={day.day}>
              <span>{day.short}</span>
              <input
                value={pattern[day.day] || ""}
                onFocus={() => setActivePatternDay(day.day)}
                ...
                placeholder={day.day === 1 || day.day === 3 ? "08-17 P30" : day.day === 2 || day.day === 4 ? "07-15 P30" : ""}
              />
            </label>
          ))}
        </div>
      ...
      <div className="panel bulk-panel">
        <div>
          ...
          <p className="subtle">Pildymas naudoja darbuotojo darbo laiko modelį. Jei modelio nėra, taikoma 08:00-17:00 P60, kad darbo diena būtų 8 val.</p>
        </div>
        <div className="bulk-actions">
          <button type="button" className="btn" onClick={autofillWorkdays}><Sparkles size={15} /> Pildyti pagal modelį</button>
          <select value={clearEmployeeId} onChange={(event) => setClearEmployeeId(event.target.value)}>
            <option value="all">Visi darbuotojai</option>
            {employees.map((employee) => <option key={employee.user_id} value={employee.user_id}>{getName(employee)}</option>)}
          </select>
          <input type="date" value={clearFrom} onChange={(event) => setClearFrom(event.target.value)} />
          <input type="date" value={clearTo} onChange={(event) => setClearTo(event.target.value)} />
            ...
            return (
              <div key={employee.user_id} className="row-fragment" style={{ display: "contents" }}>
                    ...
                    <div key={`${employee.user_id}-${dayKey(date)}`} className={`day-cell ${isWeekend(date) ? "weekend" : ""} ${isHoliday(date) ? "holiday" : ""} ${hasCellIssue ? "cell-error" : ""} ${hasCellWarning ? "cell-warning" : ""} ${reservation ? "reserved" : ""}`} onMouseEnter={() => dragCopy && dragOverCell(rowIndex, colIndex)}>
                      {isEditing ? (
                        ...
                        <button
                          ...
                          onClick={() => { setActiveCell({ row: rowIndex, col: colIndex }); setEditingCell({ row: rowIndex, col: colIndex, draft: parsed.normalized, oldValue: rawValue }); }}
                ...
                <div className="total-cell">
                  <b>{formatHours(validation?.plannedHours || 0)} val.</b>
      ...
      <style jsx>{`
        .schedule-module { display: grid; gap: 14px; padding: 18px; background: #f8fafc; color: #0f172a; }
        .schedule-toolbar, .panel, .notice, .visible-count { background: #fff; border: 1px solid #dbe4ef; border-radius: 18px; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.06); }
        .schedule-toolbar { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; padding: 16px; }
        .eyebrow { color: #047857; font-size: 12px; font-weight: 950; text-transform: uppercase; letter-spacing: .08em; }
        h2, h3 { margin: 0; letter-spacing: -0.04em; }
        h2 { font-size: 28px; }
        h3 { font-size: 18px; }
        .subtle { margin: 4px 0 0; color: #64748b; font-weight: 750; font-size: 13px; }
        .toolbar-actions, .quick-actions, .bulk-actions, .template-form, .template-list, .pattern-actions { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }
        .btn, .pill, .template-pill, select, input { border: 1px solid #cbd5e1; border-radius: 12px; min-height: 36px; padding: 0 12px; background: #fff; color: #0f172a; font-weight: 900; }
        .btn { display: inline-flex; align-items: center; gap: 7px; cursor: pointer; }
        .btn.primary { background: #047857; border-color: #047857; color: #fff; }
        .btn.ghost { background: #f8fafc; }
        .btn.danger { color: #991b1b; border-color: #fecaca; background: #fff5f5; }
        .btn:disabled { opacity: .55; cursor: not-allowed; }
        .kpi-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; }
        .kpi { min-height: 112px; text-align: left; display: grid; gap: 4px; align-content: center; padding: 16px; border-radius: 20px; background: #fff; border: 1px solid #dbe4ef; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.06); cursor: pointer; }
        .kpi svg { padding: 9px; width: 40px; height: 40px; border-radius: 14px; background: #ecfdf5; color: #047857; }
        .kpi b { font-size: 34px; line-height: 1; }
        .kpi span { color: #64748b; font-weight: 900; }
        .kpi.bad { background: #fff7f7; border-color: #fecaca; }
        .kpi.bad svg { background: #fee2e2; color: #dc2626; }
        .kpi.warn { background: #fffbeb; border-color: #fed7aa; }
        .kpi.warn svg { background: #ffedd5; color: #d97706; }
        .kpi.active { outline: 3px solid rgba(4, 120, 87, .2); }
        .panel { padding: 14px; }
        .controls-panel, .templates-panel, .bulk-panel, .pattern-panel { display: grid; grid-template-columns: minmax(250px, auto) 1fr; gap: 12px; align-items: center; }
        .label { color: #64748b; font-size: 12px; font-weight: 950; text-transform: uppercase; letter-spacing: .05em; }
        .pill { display: inline-flex; align-items: center; gap: 5px; cursor: pointer; background: #f1f5f9; }
        .help-text { justify-self: end; color: #334155; font-weight: 800; font-size: 13px; }
        .template-pill { display: inline-flex; align-items: center; gap: 6px; background: #ecfdf5; color: #065f46; border-color: #bbf7d0; cursor: pointer; }
        .checkline { display: inline-flex; align-items: center; gap: 8px; min-height: 36px; padding: 0 10px; border-radius: 12px; background: #f8fafc; font-weight: 900; color: #334155; }
        .checkline input { min-height: auto; padding: 0; }
        .pattern-grid { display: grid; grid-template-columns: repeat(7, minmax(100px,1fr)); gap: 8px; }
        .pattern-grid label { display: grid; gap: 5px; font-weight: 950; color: #64748b; font-size: 12px; }
        .pattern-grid input { width: 100%; min-width: 0; }
        .notice { display: flex; align-items: center; gap: 8px; padding: 10px 12px; color: #047857; font-weight: 900; }
        .visible-count { padding: 10px 12px; font-weight: 800; }
        .grid-shell { overflow: auto; border-radius: 18px; border: 1px solid #cbd5e1; background: #fff; box-shadow: 0 18px 44px rgba(15,23,42,.09); }
        .schedule-grid { display: grid; min-width: 1120px; }
        .head { min-height: 50px; display: flex; flex-direction: column; justify-content: center; align-items: center; border-right: 1px solid #dbe4ef; border-bottom: 1px solid #dbe4ef; background: #f1f5f9; font-weight: 950; }
        .head.weekend { background: #ffedd5; }
        .head.holiday { background: #fee2e2; color: #991b1b; }
        .head.preholiday { background: #fef3c7; }
        .head small { color: #64748b; font-size: 10px; }
        .head em { font-style: normal; color: #b45309; font-size: 9px; }
        .sticky-left { position: sticky; left: 0; z-index: 3; align-items: flex-start; padding-left: 12px; }
        .person { min-height: 62px; display: flex; flex-direction: column; justify-content: center; gap: 3px; border-right: 1px solid #dbe4ef; border-bottom: 1px solid #dbe4ef; background: linear-gradient(180deg,#fff,#f8fafc); }
        .person b { font-size: 13px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 190px; }
        .person span { color: #64748b; font-size: 11px; font-weight: 850; }
        .day-cell { min-height: 62px; padding: 2px; border-right: 1px solid #dbe4ef; border-bottom: 1px solid #dbe4ef; background: #fff; }
        .day-cell.weekend { background: #fffaf4; }
        .day-cell.holiday { background: #fff5f5; }
        .day-cell.cell-error { box-shadow: inset 0 0 0 2px rgba(220,38,38,.5); }
        .day-cell.cell-warning { box-shadow: inset 0 0 0 2px rgba(217,119,6,.38); }
        .day-cell.reserved { background-image: repeating-linear-gradient(135deg,rgba(100,116,139,.13) 0 6px,transparent 6px 12px); }
        .shift { position: relative; width: 100%; height: 56px; border: 0; border-radius: 10px; display: grid; place-items: center; cursor: pointer; font-weight: 950; color: #0f172a; }
        .shift-empty { border: 1px dashed #cbd5e1; background: #fff; color: #cbd5e1; font-size: 20px; }
        .shift-work { background: linear-gradient(135deg,#dcfce7,#86efac); color: #065f46; }
        .shift-night, .shift-warning { background: linear-gradient(135deg,#ffedd5,#fdba74); color: #9a3412; }
        .shift-duty { background: linear-gradient(135deg,#e0f2fe,#7dd3fc); color: #075985; }
        .shift-danger, .shift-unknown { background: linear-gradient(135deg,#fee2e2,#fca5a5); color: #991b1b; }
        .shift-off { background: linear-gradient(135deg,#ede9fe,#c4b5fd); color: #5b21b6; }
        .shift-vacation, .shift-reserved, .shift-shortLeave { background: linear-gradient(135deg,#dbeafe,#93c5fd); color: #1e40af; }
        .shift-sick { background: linear-gradient(135deg,#fee2e2,#fca5a5); color: #991b1b; }
        .time { display: grid; justify-items: center; gap: 1px; font-size: 8.5px; line-height: 1; transform: scaleX(.9); }
        .time i { width: 13px; height: 1px; background: currentColor; opacity: .4; }
        .time small { font-size: 8px; opacity: .8; }
        .badge { position: absolute; top: 3px; width: 14px; height: 14px; border-radius: 999px; color: #fff; display: grid; place-items: center; font-size: 8px; font-weight: 950; }
        .badge.reservation { right: 3px; background: #64748b; }
        .badge.training { left: 3px; background: #dc2626; }
        .cell-input { width: 100%; height: 56px; border: 2px solid #047857; border-radius: 10px; text-align: center; font-size: 12px; font-weight: 950; outline: none; }
        .total-cell { min-height: 62px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; border-bottom: 1px solid #dbe4ef; background: #f8fafc; }
        .total-cell b { font-size: 12px; }
        .status { font-size: 10px; font-weight: 950; max-width: 96px; text-align: center; line-height: 1.15; }
        .status.ok { color: #047857; }
        .status.warn { color: #d97706; }
        .status.bad { color: #dc2626; }
        .bottom-grid { display: grid; grid-template-columns: 1.15fr .85fr .9fr; gap: 12px; align-items: start; }
        .panel-title { display: flex; align-items: center; gap: 8px; font-weight: 950; margin-bottom: 10px; }
        .issues-list, .history-list { display: grid; gap: 7px; max-height: 260px; overflow: auto; }
        .issue { padding: 9px 10px; border-radius: 13px; border: 1px solid #dbe4ef; background: #f8fafc; font-size: 12px; }
        .issue.bad { background: #fef2f2; border-color: #fecaca; }
        .issue.warn { background: #fff7ed; border-color: #fed7aa; }
        .issue span { color: #475569; font-weight: 750; }
        .summary-grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 8px; }
        .summary-grid div { padding: 10px; border-radius: 14px; background: #f8fafc; border: 1px solid #e2e8f0; display: grid; gap: 4px; }
        .summary-grid span { color: #64748b; font-size: 11px; font-weight: 950; text-transform: uppercase; }
        .summary-grid b { font-size: 18px; }
        .history-item { display: grid; grid-template-columns: minmax(0,1fr) auto; gap: 8px; padding: 9px 10px; border-radius: 13px; background: #f8fafc; font-weight: 850; font-size: 12px; }
        .history-item span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .history-item small { color: #64748b; font-weight: 900; }
        .empty-state { color: #64748b; font-weight: 850; padding: 14px; border-radius: 14px; background: #f8fafc; }
        @media (max-width: 1100px) { .schedule-toolbar, .controls-panel, .templates-panel, .bulk-panel, .pattern-panel { grid-template-columns: 1fr; display: grid; } .kpi-grid, .bottom-grid, .pattern-grid { grid-template-columns: 1fr; } .help-text { justify-self: start; } }
      `}</style>
    </section>
  );
}
