...
function normalizeScheduleCode(value: unknown) {
  ...
  if (!raw) return { status: "", start: null as string | null, end: null as string | null }
  if (["A", "ATOSTOGOS", "VACATION"].includes(raw)) return { status: "VACATION", start: "00:00", end: "23:59" }
  if (["L", "LIGA", "SICK"].includes(raw)) return { status: "SICK", start: "00:00", end: "23:59" }
  if (["P", "POILSIS", "OFF"].includes(raw)) return { status: "OFF", start: "00:00", end: "23:59" }
  if (["D", "WORK"].includes(raw)) return { status: "WORK", start: "08:00", end: "16:00" }
  ...
  return { status: raw, start: null as string | null, end: null as string | null }
}
...
export default function TeamPage() {
    ...
    for (const employee of employees) {
      for (const requirement of requirements) {
        ...
        const latest = [...documentVerifications]
          .filter((verification) => verification.employee_id === employee.user_id && verification.document_type === requirement.document_type)
  ...
  async function addCredential() {
    ...
    setCredentialForm((prev) => ({ ...prev, type: "", number: "", issuer: "", issued_at: "", expires_at: "", note: "" }))
    await loadAll()
  ...
  async function addDocumentVerification() {
    ...
    await loadAll()
    setMessage("Dokumento patikrinimas užregistruotas. Kopija sistemoje nesaugoma.")
  }
  ...
  const scheduleComplianceRows = useMemo(() => {
    ...
    return filteredEmployees.map((employee) => {
      ...
      for (const day of scheduleDays) {
        ...
        const windowEntries = employeeEntries.filter((entry) => entry.date >= windowStart && entry.date <= windowEnd)
      ...
      const workEntries = employeeEntries.filter((entry) => entry.status === "WORK" && entry.start_datetime && entry.end_datetime)
      ...
      for (let i = 1; i < workEntries.length; i += 1) {
        ...
        if (Number.isNaN(previousEnd.getTime()) || Number.isNaN(nextStart.getTime())) continue

        const restHours = (nextStart.getTime() - previousEnd.getTime()) / 3600000
        if (restHours >= 0) shortestRestHours = shortestRestHours === null ? restHours : Math.min(shortestRestHours, restHours)
      }

      const warnings: string[] = []
      if (maxSevenDayHours > 52) warnings.push("viršija 52 val. per 7 d.")
      if (maxSevenDayWorkDays > 6) warnings.push("daugiau nei 6 darbo d. per 7 d.")
      if (shortestRestHours !== null && shortestRestHours < 11) warnings.push("poilsis tarp pamainų < 11 val.")
  ...
  return (
    <main style={styles.page}>
      ...
      <section style={styles.statsGrid}>
        ...
        <Stat title="Laukia atostogų" value={pendingVacations.length} icon={Umbrella} danger={pendingVacations.length > 0} />
        <Stat title="Baigiasi dokumentai" value={expiringCredentials.length + expiringVerifiedDocuments} icon={AlertTriangle} danger={expiringCredentials.length + expiringVerifiedDocuments > 0} />
        <Stat title="Trūksta dokumentų patikrų" value={missingDocuments} icon={BadgeCheck} danger={missingDocuments > 0} />
        <Stat title="Mokymų neatitikimai" value={trainingNonCompliantEmployees} icon={GraduationCap} danger={trainingNonCompliantEmployees > 0} />
        <Stat title="Artimiausi grafikai" value={activeSchedule.length} icon={CalendarDays} />
      </section>
      ...
      {tab === "overview" ? (
        <section style={styles.gridTwo}>
          <Card title="Artimiausi dokumentų terminai">
            <DataTable
              columns={["Darbuotojas", "Tipas", "Nr.", "Galioja iki", "Pastaba"]}
          ...
          <Card title="Artimiausi grafiko įrašai">
            <DataTable
              columns={["Data", "Darbuotojas", "Pradžia", "Pabaiga", "Statusas"]}
              rows={activeSchedule.map((item) => [
                fmt(item.date),
                employeeName(map.get(item.employee_id)),
                item.start_datetime ? new Date(item.start_datetime).toLocaleString("lt-LT") : "—",
                item.end_datetime ? new Date(item.end_datetime).toLocaleString("lt-LT") : "—",
                item.status || "—",
              ])}
      ...
      {tab === "vacations" ? (
        <Card title="Atostogų prašymai">
          ...
          <DataTable
            columns={["Darbuotojas", "Nuo", "Iki", "Dienos", "Statusas", "Veiksmas"]}
      ...
      {tab === "docs" ? (
        <Card title="Pažymos ir licencijos">
          <div style={styles.formGrid}>
            ...
            <input value={credentialForm.type} onChange={(e) => setCredentialForm({ ...credentialForm, type: e.target.value })} placeholder="Tipas: sveikatos pažyma, licencija" style={styles.input} />
            <input value={credentialForm.number} onChange={(e) => setCredentialForm({ ...credentialForm, number: e.target.value })} placeholder="Numeris" style={styles.input} />
            <input value={credentialForm.issuer} onChange={(e) => setCredentialForm({ ...credentialForm, issuer: e.target.value })} placeholder="Išdavė" style={styles.input} />
            ...
            <input value={credentialForm.note} onChange={(e) => setCredentialForm({ ...credentialForm, note: e.target.value })} placeholder="Pastaba" style={styles.input} />
          ...
          <DataTable
            columns={["Darbuotojas", "Tipas", "Nr.", "Išdavė", "Išduota", "Galioja iki", "Pastaba"]}
      ...
      {tab === "trainings" ? (
        <section style={styles.gridTwo}>
          <Card title="Mokymų dashboard">
            ...
            <DataTable
              columns={["Darbuotojas", "Mokymas", "Statusas", "Baigta", "Galioja iki", "Val.", "Patikrino", "Veiksmai"]}
          ...
          <Card title="Priminimai">
            <DataTable
              columns={["Darbuotojas", "Mokymas", "Statusas", "Galioja iki", "Priminimai"]}
      ...
      {tab === "documentChecks" ? (
        <section style={styles.gridTwo}>
          ...
          <Card title="Automatinė dokumentų atitiktis">
            ...
            <DataTable
              columns={["Darbuotojas", "Dokumentas", "Statusas", "Galioja iki", "Tikrino", "Kopija"]}
          ...
          <Card title="Patikrinimų ir audito registras">
            ...
            <DataTable
              columns={["Laikas", "Vartotojas", "Veiksmas", "Objektas", "Detalės"]}
      ...
      {tab === "inventory" ? (
        <section style={styles.gridTwo}>
          <Card title="Darbuotojams priskiriami daiktai">
            ...
            <DataTable
              columns={["Pavadinimas", "Kategorija", "Serija", "Dydis", "Statusas"]}
          ...
          <Card title="Priskirti daiktą darbuotojui">
            <div style={styles.formGrid}>
              ...
              <select value={assignmentForm.item_id} onChange={(e) => setAssignmentForm({ ...assignmentForm, item_id: e.target.value })} style={styles.input}>
                ...
                {personnelInventory.filter((item) => item.status !== "assigned").map((item) => (
      ...
      {tab === "candidates" ? (
        <Card title="Kandidatai">
          ...
          <DataTable
            columns={["Kandidatas", "Kontaktai", "Pareigos", "Statusas", "Patirtis"]}
      ...
      {selectedTrainingDetails ? (
        <div style={styles.modalBackdrop}>
          <div style={styles.modal}>
            ...
            <DataTable
              columns={["Mokymas", "Statusas", "Baigta", "Galioja iki", "Val.", "Patikrino"]}
...
function Stat({
  ...
  return (
    <article style={danger ? styles.statDanger : styles.stat}>
