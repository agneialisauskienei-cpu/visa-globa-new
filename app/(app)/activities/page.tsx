...
export default function ActivitiesPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f6f8f5] px-4 py-5 text-[#20312a] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1600px] space-y-6">
        <section className="rounded-[2rem] border border-[#dce6de] bg-white/95 p-5 shadow-[0_16px_40px_rgba(32,49,42,0.08)] lg:p-6">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#d7e2d9] bg-[#eef4ef] px-3 py-1 text-xs font-black uppercase tracking-wider text-[#355244]">
                <Sparkles className="h-3.5 w-3.5" /> Socialinės globos veiklų kalendorius
              </div>
              <h1 className="text-3xl font-black tracking-tight text-[#20312a] lg:text-5xl">Veiklos ir lankomumas</h1>
              <p className="mt-2 max-w-3xl text-sm font-semibold text-[#66756c] lg:text-base">
                Kalendoriaus vaizdas, dienos planai, veiklų kortelės ir greitas lankomumo žymėjimas.
              </p>
            </div>

            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => setMonth((previous) => addMonths(previous, -1))} className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#d8e3da] bg-white text-[#355244] transition hover:bg-[#eef4ef]">
              ...
              <button
                ...
                className="rounded-2xl border border-[#d8e3da] bg-white px-5 py-3 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef]"
              >
                Šiandien
              </button>
              <button type="button" onClick={() => setMonth((previous) => addMonths(previous, 1))} className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#d8e3da] bg-white text-[#355244] transition hover:bg-[#eef4ef]">
        ...
        <section className="grid grid-cols-1 gap-6 2xl:grid-cols-[1.2fr_0.8fr]">
          <div className="overflow-hidden rounded-[2rem] border border-[#dce6de] bg-white shadow-[0_16px_40px_rgba(32,49,42,0.08)]">
            <div className="flex flex-col gap-4 border-b border-[#dfe8e1] p-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex items-center gap-2 text-xl font-black capitalize text-[#20312a]">
                  <CalendarDays className="h-5 w-5 text-[#2f4f3f]" /> {monthLabel(month)}
                </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                <label className="relative block w-full lg:w-[300px]">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-2xl border border-[#d8e3da] bg-white py-3 pl-12 pr-4 text-sm font-bold outline-none transition placeholder:text-[#8a9a91] focus:border-[#2f4f3f] focus:ring-4 focus:ring-[#2f4f3f]/10"
                  />
                </label>

                <div className="flex rounded-2xl border border-[#d7e2d9] bg-[#eef4ef] p-1 text-sm font-black">
                  {([
                    ["day", "Diena"],
                    ["week", "Savaitė"],
                    ["month", "Mėnuo"],
                    ...
                    <button
                      ...
                      className={`rounded-xl px-4 py-2 transition ${viewMode === value ? "bg-white text-[#20312a] shadow-sm" : "text-[#557066] hover:bg-white/60"}`}
                    >
                      {label}
                    </button>
                  ))}
            ...
            {loading ? (
              <div className="p-12 text-center text-lg font-black text-[#66756c]">Kraunama...</div>
            ) : (
              <CalendarGrid
          ...
          <aside className="rounded-[2rem] border border-[#dce6de] bg-white p-5 shadow-[0_16px_40px_rgba(32,49,42,0.08)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-black text-[#20312a]">Šios dienos planas</h2>
                <p className="mt-1 text-sm font-semibold capitalize text-[#66756c]">{fullDateLabel(selectedDate)}</p>
              </div>
              <button type="button" onClick={() => openActivityModal(selectedDate)} className="rounded-2xl bg-[#2f4f3f] px-4 py-2 text-sm font-black text-white transition hover:bg-[#243d31]">
                + Pridėti
              </button>
            </div>
              ...
              {selectedDateActivities.length ? (
                selectedDateActivities.map((activity, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="pt-2 text-sm font-black text-[#66756c]">{activity.start_time?.slice(0, 5) || "—"}</div>
                ...
                <div className="rounded-3xl border border-dashed border-[#d8e3da] bg-[#f8fbf8] p-8 text-center">
                  <CalendarDays className="mx-auto h-8 w-8 text-[#8a9a91]" />
                  <p className="mt-3 font-black text-[#20312a]">Šią dieną veiklų nėra</p>
                  <button type="button" onClick={() => openActivityModal(selectedDate)} className="mt-4 rounded-2xl bg-[#2f4f3f] px-4 py-2 text-sm font-black text-white">
                    Sukurti veiklą
                  </button>
                </div>
              )}
        ...
        <section className="rounded-[2rem] border border-[#dce6de] bg-white p-5 shadow-[0_16px_40px_rgba(32,49,42,0.08)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="flex items-center gap-2 text-xl font-black text-[#20312a]"><ListChecks className="h-5 w-5 text-[#2f4f3f]" /> Lankomumo principas</h2>
              <p className="mt-1 text-sm font-semibold text-[#66756c]">
          ...
          <section className="w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-[#dfe8e1] p-6">
              <div>
                ...
                <h3 className="mt-1 text-3xl font-black tracking-tight text-[#20312a]">Kaip naudotis veiklų kalendoriumi?</h3>
              </div>

              <button
                type="button"
                onClick={() => setCalendarHelpOpen(false)}
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#eef4ef] text-[#355244] transition hover:bg-[#dfe8e1]"
            ...
            <div className="space-y-4 p-6">
              <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                <h4 className="font-black text-[#20312a]">1. Veiklos kūrimas</h4>
                <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
                  Spausk tuščią dieną kalendoriuje arba mygtuką „Nauja veikla“. Įvesk pavadinimą, datą, pradžią ir pabaigą.
                </p>
              </div>

              <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                <h4 className="font-black text-[#20312a]">2. Lankomumo žymėjimas</h4>
                <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => setCalendarHelpOpen(false)}
                  className="rounded-2xl bg-[#2f4f3f] px-5 py-3 text-sm font-black text-white transition hover:bg-[#243d31]"
                >
                  Supratau
                </button>
              </div>
      ...
      <style jsx global>{`
        ...
        .input:focus {
          border-color: #2f4f3f;
          box-shadow: 0 0 0 4px rgba(47, 79, 63, 0.12);
        }
      `}</style>
...
function CalendarGrid({
  ...
  return (
    <div>
      {viewMode !== "day" ? (
        <div className="grid grid-cols-7 border-b border-[#dfe8e1] bg-[#f8fbf8] text-center text-xs font-black uppercase tracking-wider text-[#66756c]">
          {WEEK_DAYS.map((day) => (
        ...
        {days.map((day) => {
          ...
          return (
            <button
              ...
              className={`min-h-[156px] border-b border-r border-[#e4ece6] bg-white p-3 text-left transition hover:bg-[#f8fbf8] ${
                !isCurrentMonth && viewMode === "month" ? "opacity-45" : ""
              } ${isSelected ? "ring-2 ring-inset ring-[#2f4f3f]" : ""}`}
            >
              <div className="flex items-center justify-between gap-2">
                <div className={`flex h-9 w-9 items-center justify-center rounded-2xl text-sm font-black ${isToday ? "bg-[#2f4f3f] text-white" : "bg-[#eef4ef] text-[#20312a]"}`}>
                  {day.getDate()}
                </div>
                {dayActivities.length ? <span className="rounded-full bg-[#eef4ef] px-2 py-1 text-xs font-black text-[#355244]">{dayActivities.length}</span> : null}
              </div>
              ...
              <div className="mt-3 space-y-2">
                ...
                {dayActivities.length > (viewMode === "month" ? 3 : 8) ? (
                  <div className="rounded-2xl bg-[#f1f5f2] px-3 py-2 text-xs font-black text-[#66756c]">+{dayActivities.length - 3} daugiau</div>
                ) : null}
              </div>
            </button>
          );
      ...
      <section className="w-full max-w-3xl rounded-[2rem] bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-6 border-b border-[#dfe8e1] p-6">
          <div>
            ...
            <p className="mt-2 text-sm font-semibold text-[#66756c]">Suplanuok veiklą kalendoriuje.</p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <button
              type="button"
              onClick={() => setHelpOpen(true)}
              className="inline-flex h-12 items-center gap-2 rounded-2xl border border-[#d8e3da] bg-white px-4 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef]"
            ...
            <button type="button" onClick={onClose} className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#eef4ef] text-[#355244] transition hover:bg-[#dfe8e1]" aria-label="Uždaryti">
              <X className="h-6 w-6" />
            </button>
          </div>
        </div>
            ...
            <section className="w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-[#dfe8e1] p-6">
                <div>
                  ...
                  <h3 className="mt-1 text-3xl font-black tracking-tight text-[#20312a]">Kaip pildyti naują veiklą?</h3>
                </div>
              ...
              <div className="space-y-4 p-6">
                <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                  <h4 className="font-black text-[#20312a]">1. Pavadinimas</h4>
                  <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
                ...
                <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                  <h4 className="font-black text-[#20312a]">2. Data ir laikas</h4>
                  <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
                ...
                <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                  <h4 className="font-black text-[#20312a]">4. Po sukūrimo</h4>
                  <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
        ...
        <div className="space-y-5 p-6">
          ...
          <div className="flex justify-end gap-3 border-t border-[#dfe8e1] pt-5">
            <button type="button" onClick={onClose} className="rounded-2xl border border-[#d8e3da] bg-white px-5 py-3 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef]">
              Atšaukti
            </button>
            <button type="button" onClick={onSubmit} disabled={saving || timeInvalid} className="rounded-2xl bg-[#2f4f3f] px-5 py-3 text-sm font-black text-white transition hover:bg-[#243d31] disabled:opacity-60">
      ...
      <section className="max-h-[94vh] w-full max-w-7xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-6 border-b border-[#dfe8e1] p-5 lg:p-6">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black tracking-tight text-[#20312a] lg:text-4xl">{session.title}</h2>
            <p className="mt-2 flex flex-wrap items-center gap-2 text-sm font-semibold text-[#66756c]">
              <Clock className="h-4 w-4" /> {fullDateLabel(session.session_date)} · {timeLabel(session.start_time, session.end_time)}
            </p>
          </div>
          <button type="button" onClick={onClose} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#eef4ef] text-[#355244] transition hover:bg-[#dfe8e1]" aria-label="Uždaryti">
            <X className="h-6 w-6" />
          </button>
        </div>
            ...
            <section className="w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-[#dfe8e1] p-6">
                <div>
                  ...
                  <h3 className="mt-1 text-3xl font-black tracking-tight text-[#20312a]">Kaip žymėti lankomumą?</h3>
                </div>
              ...
              <div className="space-y-5 p-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                    <h4 className="font-black text-[#20312a]">2. Kelių gyventojų žymėjimas</h4>
                    <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
                  ...
                  <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                    <h4 className="font-black text-[#20312a]">3. Kelių veiklų žymėjimas</h4>
                    <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
                  ...
                  <div className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-5">
                    <h4 className="font-black text-[#20312a]">4. Filtrai</h4>
                    <p className="mt-2 text-sm font-bold leading-6 text-[#66756c]">
        ...
        <div className="max-h-[calc(94vh-118px)] space-y-5 overflow-y-auto p-5 lg:p-6">
          ...
          <section className="rounded-3xl border border-[#dfe8e1] bg-white p-4">
            <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-sm font-black uppercase tracking-widest text-[#2f4f3f]">Taikyti į kelias veiklas</h3>
              ...
              <span className="rounded-full border border-[#d8e3da] bg-[#eef4ef] px-3 py-1 text-xs font-black text-[#355244]">
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 rounded-2xl border px-4 py-3 text-left transition ${
                      selected
                        ? "border-[#2f4f3f] bg-[#2f4f3f] text-white"
                        : "border-[#d8e3da] bg-white text-[#355244] hover:bg-[#eef4ef]"
                    }`}
                  >
                    <div className="text-sm font-black">{item.title}</div>
                    <div className={`mt-1 text-xs font-bold ${selected ? "text-white/75" : "text-[#66756c]"}`}>
          ...
          <section className="rounded-3xl border border-[#dfe8e1] bg-[#f8fbf8] p-4">
            <div className="grid gap-3 xl:grid-cols-[1fr_220px_220px]">
              <label className="relative block">
                <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#8a9a91]" />
                <input value={residentQuery} onChange={(event) => onResidentQuery(event.target.value)} placeholder="Ieškoti gyventojo..." className="h-12 w-full rounded-2xl border border-[#d8e3da] bg-white py-3 pl-12 pr-4 text-sm font-bold outline-none transition placeholder:text-[#8a9a91] focus:border-[#2f4f3f] focus:ring-4 focus:ring-[#2f4f3f]/10" />
              </label>

              <select value={roomFilter} onChange={(event) => onRoomFilter(event.target.value)} className="h-12 rounded-2xl border border-[#d8e3da] bg-white px-4 text-sm font-bold outline-none transition focus:border-[#2f4f3f] focus:ring-4 focus:ring-[#2f4f3f]/10">
              ...
              <select value={attendanceFilter} onChange={(event) => onAttendanceFilter(event.target.value as AttendanceFilter)} className="h-12 rounded-2xl border border-[#d8e3da] bg-white px-4 text-sm font-bold outline-none transition focus:border-[#2f4f3f] focus:ring-4 focus:ring-[#2f4f3f]/10">
              ...
              <div className="flex flex-wrap items-center gap-2 xl:col-span-3">
                <span className="mr-1 text-xs font-black uppercase tracking-widest text-[#66756c]">Masinis statusas:</span>
                {STATUS_OPTIONS.map((option) => (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => onQuickStatus(option.value)}
                    className={`rounded-full border px-3 py-1 text-xs font-black transition ${option.chipClass} ${quickStatus === option.value ? "ring-2 ring-[#2f4f3f] ring-offset-2" : ""}`}
                  >
                    {option.short} = {option.label}
                  </button>
                ))}
            ...
            <p className="mt-4 text-xs font-bold leading-5 text-[#66756c]">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" onClick={onApplySelected} disabled={bulkSaving} className="rounded-2xl bg-[#2f4f3f] px-4 py-2 text-sm font-black text-white transition hover:bg-[#243d31] disabled:opacity-60">
                <CheckCircle2 className="mr-2 inline h-4 w-4" /> Pritaikyti pažymėtiems ({selectedCells.size}) į pasirinktas veiklas
              </button>
              <button type="button" onClick={() => onMarkAll("attended")} disabled={bulkSaving} className="rounded-2xl border border-[#d8e3da] bg-white px-4 py-2 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef] disabled:opacity-60">
                Matomus D į pasirinktas veiklas
              </button>
              <button type="button" onClick={() => onMarkAll("absent")} disabled={bulkSaving} className="rounded-2xl border border-[#d8e3da] bg-white px-4 py-2 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef] disabled:opacity-60">
                Matomus N į pasirinktas veiklas
              </button>
              <button type="button" onClick={onCopyPrevious} disabled={bulkSaving} className="rounded-2xl border border-[#d8e3da] bg-white px-4 py-2 text-sm font-black text-[#355244] transition hover:bg-[#eef4ef] disabled:opacity-60">
          ...
          {selectedSessionIds.size > 1 ? (
            <section className="overflow-hidden rounded-3xl border border-[#dfe8e1] bg-white">
              <div className="border-b border-[#dfe8e1] bg-[#f8fbf8] px-4 py-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-black uppercase tracking-widest text-[#2f4f3f]">Mini matrica</h3>
                  <button
                    type="button"
                    onClick={() => setHelpOpen(true)}
                    className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-[#d8e3da] bg-white text-[#355244] transition hover:bg-[#eef4ef]"
                    aria-label="Kaip naudoti mini matricą"
                  >
                    <HelpCircle className="h-4 w-4" />
                  </button>
                </div>
                <p className="mt-1 text-sm font-bold text-[#66756c]">Čia galima žymėti kiekvieną gyventojo ir veiklos langelį atskirai.</p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-[760px] w-full text-left text-sm">
                  <thead className="bg-[#fbfdfb] text-xs font-black uppercase tracking-widest text-[#66756c]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#edf2ee]">
                    {residents.slice(0, 8).map((resident) => (
                      <tr key={resident.id}>
                        <td className="px-4 py-3 font-black text-[#20312a]">{residentName(resident)}</td>
                        {sessionOptions
                          .filter((item) => selectedSessionIds.has(item.id))
                          .map((item) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`inline-flex h-8 min-w-8 items-center justify-center rounded-full border px-2 text-xs font-black transition hover:scale-105 ${
                                          active ? option.cellClass : "border-[#d8e3da] bg-white text-[#66756c] hover:bg-[#eef4ef]"
                                        }`}
          ...
          <section className="overflow-hidden rounded-3xl border border-[#dfe8e1] bg-white">
            <div className="grid grid-cols-[44px_minmax(230px,1.5fr)_130px_170px] bg-[#f8fbf8] px-4 py-3 text-xs font-black uppercase tracking-widest text-[#66756c]">
              <div></div>
              <div>Gyventojas</div>
              <div>Kambarys</div>
              <div>Statusas</div>
            ...
            <div className="max-h-[480px] divide-y divide-[#edf2ee] overflow-auto">
              {residents.length ? (
                residents.map((resident) => {
                  ...
                  return (
                    <div key={resident.id} className={`grid grid-cols-[44px_minmax(230px,1.5fr)_130px_170px] items-center gap-3 px-4 py-3 transition ${selected ? "bg-[#eef4ef]" : "hover:bg-[#f8fbf8]"}`}>
                      <button
                        type="button"
                        onClick={() => onToggleSelection(resident.id)}
                        className={`flex h-8 w-8 items-center justify-center rounded-xl border text-xs font-black ${selected ? "border-[#2f4f3f] bg-[#2f4f3f] text-white" : "border-[#d8e3da] bg-white text-[#66756c]"}`}
                      ...
                      <button type="button" onClick={() => onCycleStatus(resident.id)} className="text-left">
                        <p className="font-black text-[#20312a]">{residentName(resident)}</p>
                      </button>

                      <div className="text-sm font-bold text-[#66756c]">{roomMap.get(String(resident.current_room_id || "")) || "—"}</div>
                      ...
                      <button type="button" onClick={() => onCycleStatus(resident.id)} className="text-left">
                        {meta ? (
                          ...
                          <span className="inline-flex min-w-[120px] justify-center rounded-full border border-[#d8e3da] bg-[#f8fbf8] px-3 py-1 text-sm font-black text-[#66756c]">Nepažymėta</span>
                        )}
                      </button>
                    </div>
                  );
                ...
                <div className="p-10 text-center font-bold text-[#66756c]">Gyventojų pagal filtrus nėra. Neatvykę, mirę arba visam laikui išvykę gyventojai veiklų lankomumui nerodomi.</div>
              )}
            </div>
          </section>
        </div>
...
function StatCard({ label, value, warning = false }: { label: string; value: string; warning?: boolean }) {
  return (
    <article className={`rounded-2xl border p-3 ${warning ? "border-[#ead9a8] bg-[#fff8e6]" : "border-[#dfe8e1] bg-[#f8fbf8]"}`}>
      <div className={`text-xs font-bold ${warning ? "text-[#7b5f1b]" : "text-[#66756c]"}`}>{label}</div>
      <div className={`mt-1 truncate text-lg font-black capitalize ${warning ? "text-[#6a4c12]" : "text-[#20312a]"}`}>{value}</div>
    </article>
  );
}
  ...
  return (
    <button
      ...
      className={`rounded-3xl border p-5 text-left transition hover:scale-[1.01] hover:shadow-sm ${
        active ? "border-[#2f4f3f] ring-2 ring-[#2f4f3f] ring-offset-2" : "border-slate-200"
      } ${toneClass}`}
    >
...
function Field({ label, children, full = false }: { label: string; children: ReactNode; full?: boolean }) {
  return (
    <label className={`block ${full ? "md:col-span-2" : ""}`}>
      <span className="mb-2 block text-xs font-black uppercase tracking-widest text-[#66756c]">{label}</span>
      {children}
    </label>
  );
}
