...
export default function OrganizationsPage() {
  ...
  async function loadOrganizations() {
    try {
      ...
      const enriched = await Promise.all(
        rows.map(async (org) => {
          ...
          try {
            ...
            if (error) {
              console.error('organization_members count failed:', org.id, error)
            } else {
              membersCount = count || 0
            }
          } catch (e) {
            console.error('organization_members count exception:', org.id, e)
          }
          ...
          try {
            ...
            if (error) {
              console.error('residents count failed:', org.id, error)
            } else {
              residentsCount = count || 0
            }
          } catch (e) {
            console.error('residents count exception:', org.id, e)
          }
          ...
          try {
            ...
            if (error) {
              console.error('active admin query failed:', org.id, error)
            } else {
              adminUserId = adminMembers?.[0]?.user_id || null
            }
          } catch (e) {
            console.error('active admin query exception:', org.id, e)
          }
          ...
          if (adminUserId) {
            try {
              ...
              if (error) {
                console.error('admin profile query failed:', org.id, adminUserId, error)
              } else {
                adminProfile = (profileData as AdminProfile | null) || null
              }
            } catch (e) {
              console.error('admin profile query exception:', org.id, adminUserId, e)
            }
          }
      ...
      pushToast('error', getReadableError(error))
      setOrganizations([])
    } finally {
      setLoading(false)
    }
  ...
  async function createOrganization() {
    try {
      ...
      if (!cleanName) {
        pushToast('error', 'Įstaigos pavadinimas yra privalomas.')
        return
      }
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.error || 'Nepavyko sukurti įstaigos.')
      }
      ...
      setAddress('')
      setPlan('basic')
      pushToast('success', result.message || 'Įstaiga sėkmingai sukurta.')
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.error || 'Nepavyko atnaujinti įstaigos.')
      }

      pushToast('success', result.message || 'Įstaiga sėkmingai atnaujinta.')
      closeEdit()
      await loadOrganizations()
    } catch (error) {
      pushToast('error', getReadableError(error))
  ...
  async function assignAdmin(orgId: string) {
    ...
    if (!email) {
      pushToast('error', 'Įvesk admin el. paštą.')
      return
    }
    ...
    try {
      ...
      pushToast('success', result.message || 'Veiksmas atliktas sėkmingai.')
      setAdminEmails((prev) => ({ ...prev, [orgId]: '' }))
      await loadOrganizations()
    } catch (error) {
      pushToast('error', getReadableError(error))
    } finally {
  ...
  async function archiveOrganization(org: OrganizationCard) {
    ...
    try {
      ...
      pushToast('success', result.message || 'Įstaiga archyvuota.')
      await loadOrganizations()
    } catch (error) {
      pushToast('error', getReadableError(error))
    } finally {
  ...
  async function restoreOrganization(org: OrganizationCard) {
    try {
      ...
      pushToast('success', result.message || 'Įstaiga atstatyta.')
      await loadOrganizations()
    } catch (error) {
      pushToast('error', getReadableError(error))
    } finally {
      ...
      <div style={styles.toastStack}>
        {toasts.map((toast) => (
          <div key={toast.id} style={{ ...styles.toast, ...getToastStyle(toast.type) }}>
            <div style={styles.toastText}>{toast.text}</div>
        ...
        <section style={styles.card}>
          ...
          <div style={styles.formGrid}>
            ...
            <label style={styles.field}>
              ...
              <input value={code} onChange={(e) => setCode(e.target.value)} style={styles.input} />
            </label>
            <label style={styles.field}>
          ...
          <div style={styles.actions}>
            <button type="button" onClick={() => void createOrganization()} disabled={saving} style={styles.primaryButton}>
                ...
                <article key={org.id} style={styles.orgCard}>
                  ...
                  <div style={styles.assignCard}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAdminEmails((prev) => ({ ...prev, [org.id]: e.target.value }))
                      }
          ...
          <div style={styles.modalBackdrop} onClick={closeEdit}>
            <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={styles.formGrid}>
                ...
                <label style={styles.field}>
                  ...
                  <select
                    value={editForm.plan}
                    onChange={(e) =>
                      setEditForm((prev) => (prev ? { ...prev, plan: e.target.value } : prev))
                    }
                    style={styles.input}
                  >
                    <option value="starter">Pradinis</option>
              ...
              <div style={styles.modalActions}>
                ...
                <button type="button" onClick={() => void saveEdit()} disabled={saving} style={styles.primaryButton}>
...
const styles: Record<string, React.CSSProperties> = {
  pageOuter: { width: '100%', padding: 24, boxSizing: 'border-box', position: 'relative' },
  page: { width: '100%', maxWidth: 1400, margin: '0 auto', display: 'grid', gap: 20 },
  toastStack: {
    position: 'fixed',
    top: 16,
    right: 16,
  ...
  toastText: { fontSize: 14, fontWeight: 700, lineHeight: 1.4 },
  toastClose: {
    border: 'none',
  ...
  title: { margin: 0, fontSize: 30, fontWeight: 900, color: '#0f172a' },
  subtitle: { margin: '8px 0 0', fontSize: 15, color: '#64748b', fontWeight: 600 },
  ...
  statValue: { fontSize: 32, fontWeight: 900, color: '#111827', lineHeight: 1 },
  statLabel: { fontSize: 13, color: '#6b7280', fontWeight: 700 },
  card: { background: '#fff', padding: 18, borderRadius: 18, border: '1px solid #e5e7eb', display: 'grid', gap: 16, overflow: 'hidden' },
  cardTitle: { margin: 0, fontSize: 22, fontWeight: 800, color: '#111827' },
  ...
  listHeader: { display: 'grid', gap: 12 },
  ...
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 16, width: '100%' },
  orgCard: { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: 16, display: 'grid', gap: 14, minWidth: 0 },
  orgTop: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' },
  orgTopRight: { display: 'grid', gap: 8, justifyItems: 'end', flexShrink: 0 },
  badgesRow: { display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' },
  buttonColumn: { display: 'grid', gap: 8, justifyItems: 'end' },
  orgName: { fontSize: 18, fontWeight: 800, color: '#111827', wordBreak: 'break-word' },
  orgMeta: { marginTop: 6, fontSize: 13, color: '#6b7280', fontWeight: 600, wordBreak: 'break-word' },
  orgBadge: {
    display: 'inline-flex',
    alignItems: 'center',
  ...
  infoRow: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' },
  infoLabel: { fontSize: 13, color: '#6b7280', fontWeight: 700, flexShrink: 0 },
  infoValue: { fontSize: 13, color: '#111827', fontWeight: 700, textAlign: 'right', wordBreak: 'break-word' },
  assignCard: { display: 'grid', gap: 10, borderTop: '1px solid #e5e7eb', paddingTop: 14 },
  assignTitle: { fontSize: 14, fontWeight: 800, color: '#111827' },
  modalBackdrop: {
    position: 'fixed',
    inset: 0,
