...
export async function POST(req: NextRequest, context: Params) {
  try {
    ...
    const permission = await assertOrganizationPermission({ supabase, userId, organizationId, permission: "trainings.approve" });
    ...
    await writeServerAudit({ supabase, organizationId, actorUserId: userId, tableName: "personnel_trainings", recordId: id, action: "training.approved", before: previous, after: data });
